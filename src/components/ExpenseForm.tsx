...
export function ExpenseForm() {
  ...
  const [kategoria, setKategoria] = useState<KategoriaWydatku>('INNE')
  ...
  async function clientAction(formData: FormData) {
    ...
    if (result?.error) {
      ...
      toast.success('Wniosek został zapisany pomyślnie!')
    }
  }
  ...
  return (
    <form action={clientAction} className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-2">
          ...
          <Input
            id="nazwa"
            name="nazwa"
            placeholder="np. Hotel we Wrocławiu, Bilet PKP"
            required
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-2">
          <Label htmlFor="opis" className="font-bold">Dodatkowy opis (opcjonalnie)</Label>
          <Input
            id="opis"
            name="opis"
            placeholder="Krótki komentarz do wydatku"
            className="h-12 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all bg-gray-50/50"
          />
        </div>
      </div>
